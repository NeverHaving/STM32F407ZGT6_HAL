 ...
 * @return {HAL_StatusTypeDef}  HAL 状态
 */
HAL_StatusTypeDef OLED_DrawNum(uint8_t x, uint8_t y, uint32_t num) {
    char str[12]; // 用于存放转换后的字符串，足够大以容纳32位整数
	  snprintf(str, sizeof(str), "%lu", num); // 将数字转换为字符串

    // 调用现有的 OLED_BOOL_DrawStr 函数绘制字符串
    return OLED_BOOL_DrawStr(x, y, (uint8_t *)str, OLED_BOOL_Replace);
}
